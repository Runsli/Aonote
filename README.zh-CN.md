...
- `SITE_LANGUAGE` 只切换**界面文案**（导航、按钮、版权模板、无障碍标签等），不会自动翻译 Markdown 文章正文。
- 可选值：`zh-CN`、`en`。文案定义在 `i18n.py` 的 `TRANSLATIONS` 中。
- 新增或修改翻译键时，请保持 `zh-CN` 与 `en` 的键名一致（含 `copyright_license_notices` 下的许可证键）。
- 校验键是否一致：`python i18n.py`
...
若部署在项目子路径（例如 `https://username.github.io/repo-name/`），请设置：
...
仓库包含 GitHub Actions 工作流（见 `workflows/autobuild.yml`），可按需启用 Pages 部署。
